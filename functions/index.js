...
export const getGuestFlightPlans = onCall(
  {
    region: REGION,
  },
  async (request) => {
    const access = await resolveGuestBookingAccess(request);
    const documents = await Promise.all(
      ["arrival", "departure"].map((direction) =>
        db.collection("guestFlightPlans")
          .doc(flightPlanDocumentId(access.bookingId, direction))
          .get()
      )
    );
    ...
    return {
      plans: documents
        ...
        .map((snapshot) => serializeFlightPlan(snapshot.id, snapshot.data())),
    };
  }
);

export const saveGuestFlightPlan = onCall(
  {
    region: REGION,
  ...
  async (request) => {
    ...
    let plan;
    try {
      plan = normalizeFlightPlanInput(request.data);
    } catch (error) {
      const messages = {
        INVALID_DIRECTION: "請選擇抵達東京或前往機場。",
        INVALID_FLIGHT_NUMBER: "航班號碼格式不正確。",
        INVALID_DATE: "航班日期格式不正確。",
        INVALID_TIME: "請輸入正確的航班時間。",
        INVALID_AIRPORT: "目前僅支援成田與羽田機場。",
      };
      throw new HttpsError(
        "invalid-argument",
        messages[error?.message] || "航班資料格式不正確。"
      );
    }

    const route = await computeGuestTransitRoute(plan).catch((error) => {
      logger.warn("Guest flight transit route lookup failed.", {
    ...
    const documentId = flightPlanDocumentId(access.bookingId, plan.direction);
    const reference = db.collection("guestFlightPlans").doc(documentId);
    ...
    const document = {
      ...
      scheduledAt: Timestamp.fromDate(plan.scheduledAt),
      airport: plan.airport,
      airportName: AIRPORTS[plan.airport].name,
      terminal: plan.terminal || null,
      routeStatus: route ? "available" : "unavailable",
      route,
      mapsUrl: buildFlightMapsUrl(plan),
      createdAt: existing.data()?.createdAt || now,
      updatedAt: now,
    };
    await reference.set(document);

    return { plan: serializeFlightPlan(documentId, document) };
  }
);

export const deleteGuestFlightPlan = onCall(
  {
    region: REGION,
  },
  async (request) => {
    ...
    if (direction !== "arrival" && direction !== "departure") {
      throw new HttpsError("invalid-argument", "航班方向不正確。");
    }
    await db.collection("guestFlightPlans")
      .doc(flightPlanDocumentId(access.bookingId, direction))
      .delete();
    return { success: true };
  }
...
async function resolveGuestBookingAccess(request) {
  const requestedBookingId = String(request.data?.bookingId || "").trim();
  if (request.auth?.uid) {
    const snapshot = await db.collection("users").doc(request.auth.uid).get();
    const user = snapshot.data();
    if (snapshot.exists && user?.role === "admin") {
      if (!requestedBookingId) {
        throw new HttpsError("invalid-argument", "缺少預約識別碼。");
      }
      return { bookingId: requestedBookingId };
    }
    if (
      snapshot.exists
      && user?.role === "guest"
      && user?.active === true
      && typeof user.bookingId === "string"
      && user.bookingId
      && (!requestedBookingId || requestedBookingId === user.bookingId)
    ) {
      return { bookingId: user.bookingId };
    }
    throw new HttpsError("permission-denied", "此帳號目前無法管理這筆航班資料。");
  }

  const code = String(request.data?.guestAccessCode || "")
    .replace(/[^a-zA-Z0-9]/g, "")
    .toUpperCase();
  if (!code) {
  ...
  const isValid = snapshot.exists
    ...
    && typeof access.bookingId === "string"
    && access.bookingId;
  if (!isValid || (requestedBookingId && requestedBookingId !== access.bookingId)) {
    throw new HttpsError("permission-denied", "訪客碼已失效，請重新登入。");
  }
  return { bookingId: access.bookingId };
}

async function computeGuestTransitRoute(plan) {
  const apiKey = googleMapsApiKey.value();
  if (!apiKey) return null;

  const request = routeRequestForFlight(plan);
  const response = await fetch("https://routes.googleapis.com/directions/v2:computeRoutes", {
    ...
    body: JSON.stringify({
      ...
      ...(request.arrivalTime ? { arrivalTime: request.arrivalTime } : {}),
      languageCode: "zh-TW",
      units: "METRIC",
    }),
  });
  if (!response.ok) {
    const error = new Error("ROUTES_API_FAILED");
    error.status = response.status;
    throw error;
  }
  const result = await response.json();
  return summarizeGoogleRoute(result.routes?.[0]);
}

function buildFlightMapsUrl(plan) {
  const airport = AIRPORTS[plan.airport].routeLocation;
  const isArrival = plan.direction === "arrival";
  const params = new URLSearchParams({
    api: "1",
    origin: isArrival ? airport : "東京都台東区蔵前4丁目23-7",
    destination: isArrival ? "東京都台東区蔵前4丁目23-7" : airport,
    travelmode: "transit",
  });
  return `https://www.google.com/maps/dir/?${params.toString()}`;
}

function serializeFlightPlan(id, plan) {
  return {
    id,
    ...
    flightDate: plan.flightDate,
    scheduledTime: plan.scheduledTime,
    scheduledAt: plan.scheduledAt?.toDate?.().toISOString() || null,
    airport: plan.airport,
    airportName: plan.airportName,
    terminal: plan.terminal || null,
    routeStatus: plan.routeStatus,
    route: plan.route || null,
    mapsUrl: plan.mapsUrl,
