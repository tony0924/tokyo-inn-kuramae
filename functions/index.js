...
export const sendUpcomingCheckInReminders = onSchedule(
  ...
  async () => {
    ...
    if (!adminEmails.length) {
      logger.warn("No admin emails found. Guest check-in reminders will send without CC.");
    }
...
export const sendCheckoutAdminReminders = onSchedule(
  ...
  async () => {
    ...
    for (const booking of bookings) {
      if (!booking.guestEmail) {
        logger.warn("Booking has no guest email. Skipping guest checkout reminder.", {
...
async function getNotificationSettings() {
  ...
  const hasCurrentReminderTemplates =
    Number(stored?.reminderTemplateVersion || 0) >= CURRENT_REMINDER_TEMPLATE_VERSION;
  return {
    ...DEFAULT_SETTINGS,
    ...stored,
    reminderTemplateVersion: CURRENT_REMINDER_TEMPLATE_VERSION,
    ...
    checkInReminder: {
      ...
      ...(hasCurrentReminderTemplates ? stored?.checkInReminder || {} : {}),
    },
    checkoutAdminReminder: {
      ...DEFAULT_SETTINGS.checkoutAdminReminder,
      ...(hasCurrentReminderTemplates ? stored?.checkoutAdminReminder || {} : {}),
    },
  };
}
